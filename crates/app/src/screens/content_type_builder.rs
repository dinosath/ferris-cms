...
fn FieldConfigModal(
    ...
    let parsed_expr = core_schema::parse_expression(&expression());
    let dep_list: Vec<String> = parsed_expr
        .as_ref()
        .map(|e| e.columns())
        .unwrap_or_default();
    let unknown_deps: Vec<String> = dep_list
        .iter()
        .filter(|d| !sibling_fields.contains(d))
        .cloned()
        .collect();
    let expr_error: Option<String> = parsed_expr.as_ref().err().map(|e| e.to_string());
    let dep_text = if dep_list.is_empty() {
    ...
    let finish = move |_| {
        ...
        if computed() && supports_computed {
            attr.computed = true;
            attr.expression = Some(expression().trim().to_string());
            attr.stored = Some(computed_stored());
            attr.dependencies = core_schema::parse_expression(&expression())
                .map(|e| e.columns())
                .unwrap_or_default();
            // Computed fields cannot be required/unique or carry a default.
            attr.required = false;
            attr.unique = false;
            attr.default = None;
        }
    ...
    rsx! {
        Modal { title: title, width: 640, on_close: move |e| on_close.call(e),
            ...
            if supports_computed {
                div { style: "border-top:1px solid {color::NEUTRAL_150}; padding-top:16px; margin-top:8px;",
                    div { style: "display:flex; align-items:center; justify-content:space-between;",
                        span { style: "font-size:{typography::EPSILON_SIZE}; font-weight:600; color:{color::NEUTRAL_800};", "Computed field" }
                        Toggle { checked: computed(), label: String::new(), onchange: move |v| computed.set(v) }
                    }
                    span { style: "font-size:{typography::PI_SIZE}; color:{color::NEUTRAL_500}; display:block; margin-top:4px;",
                        "Value generated by the database from an expression over other fields of this type."
                    }
                    if computed() {
                        TextField {
                            value: "{expression}",
                            label: "Expression".to_string(),
                            placeholder: "quantity * unit_price".to_string(),
                            helper: "Use field names with + - * / %, || to join text, and parentheses.".to_string(),
                            oninput: move |v| expression.set(v),
                        }
                        Dropdown {
                            label: "Storage mode".to_string(),
                            options: vec![
                                ("stored".to_string(), "Stored".to_string()),
                                ("virtual".to_string(), "Virtual".to_string()),
                            ],
                            value: "{storage_value}",
                            onchange: move |v| computed_stored.set(v == "stored"),
                        }
