...
pub struct Attribute {
    ...
    // ---- computed / database-generated columns ----
    /// When true this column is generated by the database from `expression`
    /// rather than written by the application (`GENERATED ALWAYS AS (...)`).
    #[serde(default, skip_serializing_if = "is_false")]
    pub computed: bool,
    /// SQL expression the database evaluates, referencing other fields of the
    /// same content type (e.g. `quantity * unit_price`).
    #[serde(default, skip_serializing_if = "is_none")]
    pub expression: Option<String>,
    /// Storage mode for a computed column: `Some(true)` = STORED,
    /// `Some(false)` = VIRTUAL. `None` means "backend default" (STORED where
    /// STORED is the only option, e.g. PostgreSQL).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub stored: Option<bool>,
    /// Field names this computed expression reads. Used for validation and
...
impl Attribute {
    ...
    /// Whether this field is a database-generated (computed) column.
    pub fn is_computed(&self) -> bool {
        self.computed
    }

    /// Storage mode requested for a computed column (`true` = STORED).
    /// Defaults to STORED, the only mode PostgreSQL supports.
    pub fn is_stored(&self) -> bool {
        self.stored.unwrap_or(true)
    }

    /// Whether this field's type can be a computed/generated column. Only the
    /// scalar types listed in the Content-Type Builder are allowed.
    pub fn supports_computed(&self) -> bool {
        use FieldType::*;
        matches!(
            self.attr_type,
            Integer | Biginteger | Decimal | Float | String | Text | Boolean | Date | Datetime
        )
    }
