...
pub fn validate_payload(
    ...
    for (name, attr) in &schema.attributes {
        ...
        // Computed fields are generated by the database and are read-only:
        // a payload that explicitly writes one is rejected.
        if attr.computed && present {
            errors.push(PayloadError::new(
                name.clone(),
                "computed",
                format!("Field '{name}' is computed and cannot be written"),
            ));
            continue;
        }
